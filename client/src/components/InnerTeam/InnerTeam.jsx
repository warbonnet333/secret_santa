...
const InnerTeam = (props) => {
    ...
    const fetchTeam = async (id) => {
        try {
            ...
            setState((prev) => {
                return {...prev, isPlayed, limit, name, players, id: _id, admin}
            })
    ...
    const onDelete = async (idToDelete) => {
        ...
        let result = window.confirm('Видалити гравця? Оо');
        if (!result) {
            return
        }
    ...
    return (
        <div className={st.container}>
            ...
            {isPlayed && <FindName id={newId}/>}
            {!isPlayed && <div className={st.ready_text}>Вже готові зіграти:</div>}
            <PlayersList array={players} admin={admin} onDelete={onDelete} isPlayed={isPlayed}/>
